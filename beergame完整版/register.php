...
if ($result = mysqli_query($db,$sqlloginID)) {
        if ($row=mysqli_fetch_array($result)) {
                echo "<script>alert('ID已存在!將在確認之後跳回註冊頁面'); location.href = 'http://localhost/beergame完整版/register.html';</script>";
        } else {
                ...
                echo "<script>alert('註冊成功!將在確認之後跳回登入頁面'); location.href = 'http://localhost/beergame完整版/login.php';</script>";
