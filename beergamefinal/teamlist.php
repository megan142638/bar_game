...
if ($per == 1){
    echo '<form method="post">';
    echo '<input type="submit" name="set" value="設定需求" class="btn btn-primary"/><br/></form>';
    if (isset($_POST['set']))
        header("Location: set.php");
}
else{
    echo '<form method="post">';
    echo '<input type="submit" name="create" value="創建隊伍" class="btn btn-primary"/><br/></form>';
}

echo '<form method="post" >';
echo '<input type="submit" name="logout" value="登出" class="btn btn-primary"/></form>';

echo '<form method="post" >';
echo '<input type="submit" name="rank" value="排行榜" class="btn btn-primary"/></form>';
